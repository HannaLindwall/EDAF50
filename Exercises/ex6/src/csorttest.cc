...
void csort(Iterator first, Iterator last) {
	...
	// ex: range 9-5=4, men behöver 5 platser: 5,6,7,8,9, därav +1
	size_t rangeSize = max - min + 1;
	std::vector<unsigned int> ordered(rangeSize);
	// använd inte count här, tar alldeles för lång tid, använd counter istället
	// när man gör en for loop med Iterator så går man endast igenom de existerande
	// värdena istället för alla värden mellan min och max
	// ---for (signed int i = min; i <= max; i++ ) {
	// 	size_t occurance = std::count(first, last, i);---
	for(Iterator it = first; it != last; ++it) {
		// om min:-5 hamnar den på 0, medan 5 hamnar på 10
		// ---ordered[i - min] = occurance;---
		ordered[*it - min]++;
	}
	for (std::vector<unsigned int>::size_type j = 0; j < ordered.size(); ++j ) {
		//om det är 0 går den inte in i får loopen
		for(size_t k = 0; k < ordered[j]; ++k) {
			//översätta tillbaka från 0 till -5 och 10 till 5
			*first++ = j + min;
		}
	}
}
